...
### Asynchronous durable commits

A third mode, `AfterCommitState.KEEP_OPEN_ASYNC_COMMIT`, is the middle ground between the two: every threshold crossing produces a **real revision** — durable, chained, queryable — but the expensive half of the commit protocol runs on a background thread. The writer thread serializes the epoch's pages and hands off; the background thread performs the durability barriers (data force, index-catalogue fsync, DSYNC uber-page beacon writes) and only *then* publishes the revision to readers (**durable-before-visible** — a reader can never open a revision that a crash could still lose). Commits are pipelined with depth 1: the next epoch is built on the pending revision while the previous one hardens, and backpressure stalls the writer only if the device falls behind. A crash loses at most the in-flight epoch plus the current working set — versus everything since the last real commit with the pre-flush, and at most one batch with synchronous auto-commits.
