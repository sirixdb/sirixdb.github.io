...
doctitle: Operations & Production Deployment
title: SirixDB - Operations
---

[Edit this page on GitHub](https://github.com/sirixdb/sirixdb.github.io/edit/master/docs/operations.md)
...
usage. For API documentation, see [REST API](/docs/rest-api.html) and
[JSONiq API](/docs/jsoniq-api.html); for storage-format internals, see
...
| **Native image** | Supported via GraalVM `native-image` for `sirix-rest-api` and `sirix-kotlin-cli`. See [docs/NATIVE_IMAGE.md](https://github.com/sirixdb/sirix/blob/main/docs/NATIVE_IMAGE.md) in the source repo. |
...
The same set is applied in the project's
[Gradle build](https://github.com/sirixdb/sirix/blob/main/build.gradle) and in
the REST API CI workflow.
...
- `-XX:-UseJVMCICompiler` — workaround for a Graal JIT speculation bug
  ([oracle/graal#13387](https://github.com/oracle/graal/issues/13387)) that
  caused 27% wall-clock regressions on certain query shapes.
...
[`MetricsHandler.kt`](https://github.com/sirixdb/sirix/blob/main/bundles/sirix-rest-api/src/main/kotlin/io/sirix/rest/MetricsHandler.kt).
...
6. **Large-scale (Chicago-scale) ingestion tests are not in CI.** The reference
   ...
   caught manually by removing the `@Disabled` annotation and running locally
...
- [Architecture](/docs/architecture.html) — page format, versioning, transaction model.
- [Features](/docs/features.html) — the user-facing feature matrix.
- [REST API](/docs/rest-api.html) — endpoint reference.
- [JSONiq API](/docs/jsoniq-api.html) and [Function Reference](/docs/jsoniq-functions.html) — query layer.
- Cost-based optimizer design ([sirix repo](https://github.com/sirixdb/sirix/blob/main/docs/cost-based-optimizer-design.md)).
- Native image build ([sirix repo](https://github.com/sirixdb/sirix/blob/main/docs/NATIVE_IMAGE.md)).
- Roadmap ([sirix repo](https://github.com/sirixdb/sirix/blob/main/ROADMAP.md)).
